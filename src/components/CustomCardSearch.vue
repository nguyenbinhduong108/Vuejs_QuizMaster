<template>
  <v-card
    ...
    :key="question.id"
    color="#fff"
    v-intersect="checkLastQuestion(question)"
...
const emits = defineEmits(["click", "callMoreData"]);

function selectedCardOnClick(questionId: string) {
  emits("click", questionId);
}

const questionList = ref(props.questions);
function checkLastQuestion(question: questionProps) {
  if (
    questionList.value &&
    question === questionList.value[questionList.value?.length - 1]
  ) {
    emits("callMoreData");
  }
}
